...
await registry.recordSuccess('operator-001', transactionId);
const afterFailure = await registry.recordFailure('operator-001');
assert.equal(afterFailure.status, 'ONLINE');
assert.equal(afterFailure.health?.recentSuccessfulTransactions[0], transactionId);
assert.equal(afterFailure.health?.failureRate, 0.5);
assert.equal((await registry.heartbeat('operator-001', { stxBalanceMicroStx: 0n })).status, 'UNHEALTHY');
