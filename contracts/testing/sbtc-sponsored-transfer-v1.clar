;; Clarinet-only harness copy of the testnet adapter.
;; The production adapter pins the canonical testnet sBTC principal; this copy
;; points at the local mock token so simnet can execute the transfer path.

(define-constant SBTC_TOKEN 'ST1PQHQKV0RJXZFY1DGX8MNSNYVE3VGZJSRTPGZGM.mock-sbtc-token)
...
(define-public
  ...
  (begin
    ...
    (let ((sponsor (unwrap! tx-sponsor? ERR_SPONSOR_REQUIRED)))
      (settle amount recipient sponsor sponsor-fee quote-id expiry-height memo))))
